...
\begin{abstract}
Genomic prediction models for plant and animal breeding are trained almost universally to minimize the mean squared error, yet they are evaluated, and used to select candidate genotypes, by the Pearson correlation between predicted and observed phenotypes and by candidate ranking. We characterize correlation-consistent training objectives for neural genomic prediction, using the elementary identity that maximizing the correlation is equivalent to minimizing the mean squared error of standardized variables as a numerically stable training target, and we frame a closed-form post-hoc affine recalibration that restores predictive scale and offset while leaving the correlation and all rankings unchanged whenever its fitted slope is positive. Holding architecture, data splits and tuning fixed so that only the loss differs, across 101 trait-datasets from 12 species we find that correlation and concordance losses raise predictive ability over the squared-error baseline by roughly 0.04 in correlation and improve selection-aware metrics. The benefit is strongly architecture-dependent: large for Transformer and convolutional networks, but negligible for a well-tuned multilayer perceptron. Among the losses, only the concordance loss reliably lowers calibrated error; the correlation and hybrid losses leave it statistically unchanged once affine calibration restores scale. The advantage weakens, and on some metrics reverses, under harder leave-family-out and cross-environment splits, although these contrasts are underpowered. No neural loss unseated ridge regression or genomic best linear unbiased prediction on mean rank. Correlation-consistent training is thus a principled, well-characterized refinement to be applied judiciously, not a universal replacement for squared-error training.
\end{abstract}
...
trait--datasets from 12 species, with architecture, splits and tuning held
...
trait--dataset combinations across 12 species. (i) \emph{EasyGeSe}
...
20{,}000 markers for the two ultra-high-dimensional panels.
...
correlation $1.0000$). We also include ridge regression, elastic net, random
forest and gradient-boosted trees \citep{chen2016xgboost}. The neural predictors
...
$\times$ 3 repeats), repeated $K$-fold for wheat and SoyNAM, leave-family-out for
...
We report bootstrap (BCa) confidence intervals, paired Wilcoxon tests with
...
\texttt{ccgp} (PyTorch, scikit-learn, XGBoost), with fixed seeds, the exact CV
...
All datasets are public: EasyGeSe (Zenodo record 15348871), the CIMMYT wheat panel
(\texttt{BGLR} R package) and SoyNAM (\texttt{SoyNAM} R package). Code, fixed
...
\input{sec_results_g3.tex}

\input{sec_sorghum_g3.tex}
...
\renewcommand{\familydefault}{\sfdefault}\normalfont
\centering
\caption{\bf The 101 trait--dataset combinations across 12 species (10 from EasyGeSe,
plus CIMMYT wheat and SoyNAM). $n$ is the number of genotypes, $p$ the markers used
