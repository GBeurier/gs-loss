...
slightly degraded ($\Delta$NDCG@10 $=-0.0042$, percentile-bootstrap CI $[-0.0089,-0.0005]$
...
environment-level extrapolation rather than persisting. Moreover, every splits
loss-contrast is computed on the MLP alone --- in the splits experiment the CNN was
run only with the Pearson loss and so has no MSE baseline from which to form a
$\Delta$ --- and the MLP is precisely the architecture with the smallest random-CV
benefit, so these hard-split contrasts likely understate the CNN and Transformer
effect and should be read as suggestive. The number of genuinely
