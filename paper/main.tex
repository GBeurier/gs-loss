...
when $n\geq12$; the percentile bootstrap otherwise, as for the $n=8$ sorghum
contrast), paired Wilcoxon tests with
...
datasets are public and download automatically. Supporting Information (File~S1,
\texttt{supplement.pdf}) contains Tables~S1--S11 and Figures~S1--S5, generated
directly from the deposited result files: the numerical verification of the
equivalences (Experiment~A), the hyper-parameter search spaces and selected
configurations, the complete $\Delta$-versus-MSE tables for every predictive and
selection metric (pooled and per architecture), the confirmatory mixed-model
coefficients, mean method ranks, calibration diagnostics, the selection-at-$k$
analysis, the minibatch-size study (Experiment~E), the difficult-splits study
(Experiment~F) and a per-(dataset, trait) results table.
